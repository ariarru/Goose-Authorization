...
export default function MapContainer({width, height, latitute, longitude, zoomIn, datas}){
      ...
      return(
        <div className={`flex gap-[2px] bg-white/70 m-8`} >
          <div className={`relative ${width} ${height} border border-transparent`}>
            <ClickMap setClickMapObject={setMapObject} lat={latitute} long={longitude} z={zoomIn} data={datas} />
          </div>
        </div>
    );
    }
    ...
    return(
        <div className={`flex gap-[2px] bg-white/70 m-8`} >
          <div className={`relative ${width} ${height} border border-transparent`}>
            <ClickMap setClickMapObject={setMapObject} lat={latitute} long={longitude} z={zoomIn} />
          </div>
        </div>
    );
}